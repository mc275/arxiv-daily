...
def get_daily_papers(topic, query="slam", max_results=2, retries=3):
    """
    @param topic: str
    @param query: str
    @param max_results: int
    @param retries: number of retry attempts on failure
    @return paper_with_code: dict, content_to_web: dict, status: str
    ...
    for attempt in range(retries):
        try:
            client = arxiv.Client(page_size=max_results, delay_seconds=1.2)
            search = arxiv.Search(
                query=query,
            ...
            return data, data_web, "success"

        except Exception as e:
            logging.warning(f"ArXiv search failed (attempt {attempt + 1}/{retries}): {e}")
            if attempt < retries - 1:
                # 指数退避: 3s, 9s, 27s + 随机
                sleep_time = (3 ** (attempt + 1)) + random.random()
                logging.warning(f"Retrying in {sleep_time:.1f} seconds")
                time.sleep(sleep_time)
            else:
                logging.error(f"ArXiv search failed after {retries} attempts for topic {topic}")
                # 判断是否为限流
                err_str = str(e).lower()
                if "429" in err_str or "rate limit" in err_str or "too many requests" in err_str:
                    return {topic: {}}, {topic: {}}, "rate_limited"
...
def demo(**config):
    ...
    if config['update_paper_links'] == False:
        ...
        for topic, keyword in keywords.items():
            ...
            data, data_web, status = get_daily_papers(topic, query = keyword,
                                            max_results = max_results)
            data_collector.append(data)
            data_collector_web.append(data_web)
            topic_status[topic] = {
                "status": status,
                "count": len(list(data.values())[0]) if data.get(topic) else 0
            }
            print("\n")
            # 随机 4-8 秒间隔，避免人机检测
            sleep_time = 4 + random.random() * 4
            logging.info(f"Sleeping: {sleep_time:.1f} seconds before next topic")
            time.sleep(sleep_time)
    ...
    if topic_status:
        # 计算总体状态
        statuses = [v["status"] for v in topic_status.values()]
        if all(s == "success" for s in statuses):
            overall = "all_success"
        elif all(s in ("rate_limited", "error") for s in statuses):
            overall = "all_failed"
        else:
            overall = "partial"

        # 获取 docs 目录路径（从 json_readme_path 推导）
        docs_dir = pathlib.Path(config['json_readme_path']).parent.resolve()
        status_file = docs_dir / "arxiv-daily-status.json"
        status_report = {
            "overall": overall,
        ...
        logging.info(f"Status report written to {status_file}: overall={overall}")
